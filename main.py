...
app.config['SECRET_KEY'] = 'secret_key'  # Ключ для хеширования паролей
...
@app.route("/")
def index():
    return render_template('index.html', title='Favorit Motors')
...
def posts():
    ...
    s = []
    for i in news:
        x = [i.title, i.content, i.user.name, i.user_id]
        ...
        s.append(x)
    sp = list(reversed(s))
    return render_template("posts.html", news=sp, title='Объявления')
...
def profile(id):
    ...
    us = [user.name, user.about, user.email]
...
def add_news():
    ...
    if form.validate_on_submit():
        if len(form.title.data) > 20:
            return render_template('news.html', title='Добавление объявления',
                                   form=form,
...
def edit_news(id):
    ...
    if form.validate_on_submit():
        if len(form.title.data) > 20:
            return render_template('news.html', title='Редактирование объявления',
                                   form=form,
                                   message="*Максимальная длинна заголовка - 20 символов")
        if len(form.content.data) > 300:
            return render_template('news.html', title='Редактирование объявления',
        ...
        if news:
            ...
            abort(404)
    return render_template('news.html', title='Редактирование объявления', form=form)
